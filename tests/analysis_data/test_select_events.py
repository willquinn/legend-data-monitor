...
plot_settings = {
    ...
    "range": [None, None],
    "saving": "append",
    "plt_path": "prova/tmp-auto/generated/plt/phy/p14/r005/l200-p14-r005-phy",
}
dataset_info = {
    "experiment": "L200",
    "period": "p14",
    ...
    "type": "phy",
    "timestamps": ["20250619T213019Z"],
}


def test_select_events_pulser(sample_data):
    obj = AnalysisData(
        selection=plot_settings | dataset_info, sub_data=sample_data.copy()
    )
    obj.select_events()
...
def test_select_events_phy(sample_data):
    ...
    assert (
        (~obj.data["flag_pulser"])
        | (~obj.data["flag_fc_bsln"])
        | (~obj.data["flag_muon"])
    ).all()
