...
@pytest.fixture
def mock_helpers():
    with patch("legend_data_monitor.monitoring.add_calibration_runs") as mock_add_calib, \
         patch("legend_data_monitor.monitoring.utils.get_tiers_pars_folders") as mock_get_tiers, \
         patch("legend_data_monitor.monitoring.get_calib_data_dict") as mock_get_calib_data:

        mock_add_calib.side_effect = lambda period, run_list: run_list
        mock_get_tiers.return_value = (["tier1", "tier2"], ["pars1", "pars2"])
        
        def dummy_get_calib_data(calib_data, channel_info, tiers, pars, period, run, tier, key_result, fit):
...
def test_get_calib_pars(mock_helpers):
    ...
    # check returned keys
    expected_keys = [
        "fep", "fep_err", "cal_const", "cal_const_err",
        "run_start", "run_end", "res", "res_quad",
        "cal_const_diff", "fep_diff" # new keys
    ]
    for key in expected_keys:
        assert key in result

    # check that arrays have correct length = number of inspected calib runs
    for key in ["fep", "fep_err", "cal_const", "cal_const_err",
                "run_start", "run_end", "res", "res_quad",
                "cal_const_diff", "fep_diff"]:
    ...
    assert np.isclose(result["cal_const_diff"][0], 0.)
    assert np.isclose(result["fep_diff"][0], 0.)
