...
def check_removed(series, start_str, stop_str):
    ...
    assert not any((series.index >= start) & (series.index <= stop)), \
...
def test_filter_by_period_removes_ignore_ranges():
    ...
    # p03
    for start, stop in zip(IGNORE_KEYS["p03"]["start_keys"], IGNORE_KEYS["p03"]["stop_keys"]):
        check_removed(filtered, start, stop)
    # p04
    for start, stop in zip(IGNORE_KEYS["p04"]["start_keys"], IGNORE_KEYS["p04"]["stop_keys"]):
        check_removed(filtered, start, stop)
