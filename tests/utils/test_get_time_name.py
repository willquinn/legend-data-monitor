...
def test_run_single():
    ...
    with patch("legend_data_monitor.utils.get_multiple_run_id", return_value="r010") as mock_run:
        result = get_time_name(user_time_range)
...
def test_run_multiple():
    ...
    with patch("legend_data_monitor.utils.get_multiple_run_id", return_value="r010_r014") as mock_run:
        result = get_time_name(user_time_range)
        mock_run.assert_called_once_with(user_time_range)
...
def test_invalid_time_selection():
    ...
    with patch("legend_data_monitor.utils.logger") as mock_logger:
        result = get_time_name(user_time_range)
        mock_logger.error.assert_called_once_with("\033[91mInvalid time selection!\033[0m")
