...
class ParamData():
    ...
    def __init__(self, subsys, param, plot_settings):
        ...
        if self.param in subsystem.SPECIAL_PARAMETERS:
            ...
            params_to_get += (subsystem.SPECIAL_PARAMETERS[param] if subsystem.SPECIAL_PARAMETERS[param] else [])
        else:
        ...
        if param == 'wf_max_rel':
            ...
            self.data = self.data.set_index('datetime').groupby('channel').resample(self.sampling).count()['channel'].to_frame(name = 'event_rate').reset_index()
