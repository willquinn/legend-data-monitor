...
def read_geds(geds_dict):
    ...
    string_tot = list(geds_list[i*k+min(i, m):(i+1)*k+min(i+1, m)] for i in range(n))
    ...
    while idx+1 < len(par_average):
          ...
          if low_lim != "null":
             if par_average[idx] < low_lim:
                thr_flag = 1 # problems!
                j = 0
                time1 = datetime.fromtimestamp(times_average[idx]).strftime('%d/%m %H:%M:%S')
                while par_average[idx+j] < low_lim and idx+j+1 < len(par_average):
                      j = j+1
                idx = idx + j -1
                time2 = datetime.fromtimestamp(times_average[idx]).strftime('%d/%m %H:%M:%S')
                if time1 == time2:
                   logging.warning(f' "{parameter}"<{low_lim} {units} (at {time1}) for ch={detector}')
                else:
                   logging.warning(f' "{parameter}"<{low_lim} {units} ({time1} -> {time2}) for ch={detector}')
             idx = idx + 1 
          ...
          if upp_lim != "null":
             if par_average[idx] > upp_lim:
                ...
                time2 = datetime.fromtimestamp(times_average[idx]).strftime('%d/%m %H:%M:%S')
                if time1 == time2:
                   logging.warning(f' "{parameter}">{upp_lim} {units} (at {time1}) for ch={detector}')
                else:
                   logging.warning(f' "{parameter}">{upp_lim} {units} ({time1} -> {time2}) for ch={detector}')
             idx = idx + 1
...
def build_utime_array(raw_files, detector, det_type):
    ...
    if det_type == 'spms': utime_array = load_shifted_times(raw_files, 'spms', detector)
    if det_type == 'geds': utime_array = load_shifted_times(raw_files, 'geds', detector)
...
def load_shifted_times(raw_files, det_type, detector):
    ...
    if isinstance(raw_files, list): 
       for raw_file in raw_files:
           tmp_array = lh5.load_nda(raw_file, ['runtime'], detector+"/raw")['runtime']
           utime_array = np.append(utime_array, add_offset_to_timestamp(tmp_array, raw_file))
    else:
       tmp_array = lh5.load_nda(raw_files, ['runtime'], detector+"/raw")['runtime']
       utime_array = np.append(utime_array, add_offset_to_timestamp(tmp_array, raw_files))
...
def time_analysis(utime_array, par_array, time_cut):
    ...
    if len(time_cut) == 4:
       start_index, end_index = timecut.min_max_timestamp_thr(utime_array.tolist(), time_cut[0]+" "+time_cut[1], time_cut[2]+" "+time_cut[3])
       ...
       if end_index != end_index or start_index != start_index:
          return [], []
       if len(utime_array)!= 0 : utime_array = timecut.cut_array_in_min_max(utime_array, start_index, end_index)
       if len(par_array)!= 0   : par_array = timecut.cut_array_in_min_max(par_array, start_index, end_index)
    # last X hours analysis
    if len(time_cut) == 3:
       start_index = timecut.min_timestamp_thr(utime_array.tolist(), time_cut)
       if len(utime_array)!= 0 : utime_array = timecut.cut_array_below_min(utime_array, start_index)
       if len(par_array)!= 0   : par_array = timecut.cut_array_below_min(par_array, start_index)
...
def puls_analysis(raw_file, detector, det_type):
    ...
    det_and_puls_ievt = lh5.load_nda(raw_file, ['eventnumber'], detector+'/raw')['eventnumber'] # detector+pulser events
    puls_ievt = lh5.load_nda(raw_file, ['eventnumber'], 'ch000/raw')['eventnumber'] # not all events are pulser events
    pulser_entry = []
    not_pulser_entry = []
