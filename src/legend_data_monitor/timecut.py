...
def build_timecut_list(time_window, last_hours):
    ...
    if time_window['enabled'] == True and last_hours['enabled'] == True:
       print('Both "time_window" and "last_hours" are enabled. You must enable just one of the two analysis!')
       sys.exit(1)
...
def time_dates(time_cut):
    ...
    if len(time_cut) == 4:
       ...
       start = start_date[2] + start_date[1] + start_date[0] + 'T' + start_time[0] + start_time[1] + start_time[2] + 'Z'
       end   = end_date[2] + end_date[1] + end_date[0] + 'T' + end_time[0] + end_time[1] + end_time[2] + 'Z'
    if len(time_cut) == 3:
       end = datetime.now().strftime("%Y%m%dT&H%M%SZ")
       start = (datetime.now() - timedelta(days=time_cut[0], hours=time_cut[1], minutes=time_cut[2])).strftime("%Y%m%dT%H%M%SZ")
...
def min_timestamp_thr(timestamp, prod_time):
    ...
    thr_timestamp = (datetime.now() - timedelta(days=prod_time[0], hours=prod_time[1], minutes=prod_time[2])).timestamp()
    start_t = 0
    for t in timestamp:
          if t < thr_timestamp:
             continue
...
def cut_min_max_filelist(runs, time_cut):
   ...
   timecut_low = int(date_string_formatting(time_cut[0]) + hour_string_formatting(time_cut[1]))
   timecut_high = int(date_string_formatting(time_cut[2]) + hour_string_formatting(time_cut[3]))
...
def cut_below_threshold_filelist(runs, time_cut):
   ...
   time_difference = (datetime.now() - timedelta(days=time_cut[0], hours=time_cut[1], minutes=time_cut[2]))
   threshold_lowcut = int(time_difference.date().strftime('%Y%m%d') + time_difference.time().strftime('%H%M%S'))   
   threshold_highcut = int(datetime.now().date().strftime('%Y%m%d') + datetime.now().time().strftime('%H%M%S'))   
...
def cut_array_in_min_max(val_array, start_index, end_index):
    ...
    val_list = val_array.tolist()
    if end_index==len(val_array)-1 : val_list = val_list[start_index:]
    else                           : val_list = val_list[start_index:end_index]
    val_array = []
    val_array = np.array(val_list)
