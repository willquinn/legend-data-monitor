            ...
            if k2=='string':
              for k3,v3 in v2.items():
                  if k3=='position' and v3!='--':
                     if int(v3)>max_length: max_length=int(v3)
...
def geds_map(det_dict, string_entries, string_name, cmap_dict, map_path, pdf):
    ...
    status_map = sns.heatmap(data=dataframe, annot=labels, vmin=0, vmax=len(custom_cmap), yticklabels=y_axis_labels,
                             xticklabels=x_axis_labels, fmt='s', cmap = custom_cmap, cbar=True,
                             linewidths=1, linecolor='white', square=True, rasterized=True)
...
def spms_map(det_dict, string_entries, string_name, cmap_dict, map_path, pdf):
    ...
    for k1,v1 in det_dict.items():
        for k2,v2 in v1.items():
            if k2=='daq':
              for k3,v3 in v2.items():
                  if k3=='card':
                     if k1 in string_entries[0] or k1 in string_entries[1]: spms_ob_dict[k1] = f'\n{k1}\n{v3}-'
                     else: spms_ib_dict[k1] = f'\n{k1}\n{v3}-'
                  if k3=='ch_orca':
                     if k1 in string_entries[0] or k1 in string_entries[1]: spms_ob_dict[k1] += f'{v3}'
                     else: spms_ib_dict[k1] += f'{v3}'
    ...
    status_map = sns.heatmap(data=df_ob, annot=labels_ob, vmin=0, vmax=len(custom_cmap), yticklabels=y_lab,
                             xticklabels=x_lab_ob, fmt='s', cmap = custom_cmap, cbar=True,
                             linewidths=10, linecolor='white', square=False)
    ...
    #                blue        red        grey       white
    custom_cmap = ['#318CE7', '#CC0000', '#A9A9A9']
    status_map = sns.heatmap(data=df_ib, annot=labels_ib, vmin=0, vmax=len(custom_cmap), yticklabels=y_lab,
                             xticklabels=x_lab_ib, fmt='s', cmap = custom_cmap, cbar=True,
                             linewidths=10, linecolor='white', square=False)
