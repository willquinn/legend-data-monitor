...
class Dataset():
    def __init__(self, config):
        ...
        self.qc_name = 'isQC_flag' if config.dataset.version > 'v06.00' else 'QualityCuts_flag'

        # type of data (phy or cal or both, always list)
        self.type = config.dataset.type
        print('Data type: ' + ','.join(self.type))

        self.period = config.dataset.period
        ...
        # avoid for now because now type phy/cal can be multiple phy+cal, need to change this in that case
    ...
    def get_user_time_range(self, config):
        ...
        if 'start' in config.dataset.selection:
            ...
            for point in time_range:
                ...
                time_range[point] = ''.join(date.split('/')) + 'T' + ''.join(time.split(':')) + 'Z'
            ...
            for idx in index:
                ...
                time_range[index[idx]] = lst[idx] if field == 'timestamps' else 'r' + str(lst[idx]).zfill(3)
...
def get_run(dsp_fname: str):
    ...
    >>> get_run(l60-p01-r027-phy-20221002T192555Z-tier_dsp.lh5)
    'r027'
