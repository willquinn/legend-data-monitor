...
def get_dataloader_timerange(**kwargs):
    ...
    if 'start' in user_selection:
        ...
        for point in ['start', 'end']:
            try:
                time_range[point] = datetime.strptime(user_selection[point], '%Y-%m-%d %H:%M:%S').strftime('%Y%m%dT%H%M%SZ')
            except:
        ...
        try:
            days, hours, minutes = re.split(r'd|h|m', user_selection['window'])[:-1] # -1 for trailing ''
        except:
            logging.error("Invalid window format!")
            return
            
        dt = timedelta(days=int(days), hours=int(hours), minutes=int(minutes))
        time_range['start'] = (datetime.strptime(time_range['end'], "%Y%m%dT%H%M%SZ") - dt).strftime("%Y%m%dT%H%M%SZ")

    # -------------------------------------------------------------------------
    #  in these cases, DataLoader will be called with (timestamp/run = ...) or (timestamp/run= ...)
    # -------------------------------------------------------------------------
    elif 'timestamps' in user_selection:
        time_range = user_selection['timestamps'] if isinstance(user_selection['timestamps'], list) else [ user_selection['timestamps'] ]

    elif 'runs' in user_selection:
        runs = user_selection['runs'] if isinstance(user_selection['runs'], list) else [ user_selection['runs'] ]
        # check validity
        for run in runs:
        ...
        for plot in conf['subsystems'][subsys]:
            ...
            for field in OPTIONS:
                ...
                if not field in plot_settings:
                    logging.error(f"Provide {field} in plot settings of '{plot}' for {subsys}!")
                ...
                if not opt in OPTIONS[field]:
                    logging.error(f"Option {opt} provided for {field} in plot settings of '{plot}' for {subsys} does not exist!")
                    logging.error('Available options: {}'.format(','.join(OPTIONS[field])))
                    return False

            # if vs time was provided, need time window
            if plot_settings['plot_style'] == 'vs time' and not 'time_window' in plot_settings:
                logging.error(f"You chose plot style 'vs time' and did not provide 'time_window'!")
                return False
