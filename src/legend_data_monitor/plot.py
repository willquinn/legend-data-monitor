...
import analysis, parameters, timecut
...
def plot_parameters(ax, par_array, utime_array, detector, det_type, parameter):
    ...
    status = analysis.check_par_values(times_average, par_average, parameter, detector, det_type)
...
def plot_par_vs_time(raw_files, det_list, parameter, time_cut, det_type, string_number, det_dict, pdf=None):
    ...
    for raw_file in raw_files:
        ...
        for detector in det_list:
            ...
            if raw_file == raw_files[0]:
               if det_type == 'spms': handle_list.append(mpatches.Patch(color=j_plot[2][str(detector)], label=f'{detector} - FC: {card},{ch_orca} ({crate})'))
               if det_type == 'geds': handle_list.append(mpatches.Patch(color=j_plot[3][detector], label=f'{detector} - FC: {card},{ch_orca} ({crate})'))

            # plot detectors of the same string
            par_np_array, utime_array = parameters.load_parameter(parameter, raw_file, dsp_file, detector, det_type, time_cut)

            # to handle particular cases where the timestamp array is outside the time window:
            if len(par_np_array) == 0 and len(utime_array) == 0: continue

            start_time, end_time, status, ax = plot_parameters(ax, par_np_array, utime_array, detector, det_type, parameter)

            # fill the map with status flags
            if det_type == 'spms': detector = str(detector)
            if detector not in map_dict: map_dict[detector] = status
            else:
               if map_dict[detector] == 0: map_dict[detector] = status
    ...
    locs = np.linspace(dates.date2num(start_times[0]), dates.date2num(end_times[-1]), 10)
    xlab = '%d/%m'
    if j_config[10]['frmt'] == 'day/month-time' : xlab = '%d/%m\n%H:%M'
    if j_config[10]['frmt'] == 'time'           : xlab = '%H:%M'
    labels = [dates.num2date(loc, tz=local_timezone).strftime(xlab) for loc in locs]
    ax.set_xticks(locs)
    ax.set_xticklabels(labels)
    plt.xticks(locs, labels)
    plt.xticks(rotation=45, ha='center')
    ax.legend(loc=(1.04, 0.0), ncol=1, frameon=True, facecolor='white', framealpha=0, handles=handle_list)
    ax.grid(axis='both')
    plt.legend(loc=(1.04, 0.0), ncol=1, frameon=True, facecolor='white', framealpha=0, handles=handle_list)
    plt.xticks(rotation=45, ha='center')
    plt.grid(axis='both')
    ylab = j_par[0][parameter]['label']
    if j_par[0][parameter]['units'] != "null": ylab = ylab + ' [' + j_par[0][parameter]['units'] + ']'
    ...
    if len(time_cut) != 0:
       ...
       if det_type=='geds': pkl_name = exp + '-' + period + '-' + run + '-' + datatype + '-' + start + '_' + end + '-' + parameter + '-string' + string_number + '.pkl'
       if det_type=='spms': pkl_name = exp + '-' + period + '-' + run + '-' + datatype + '-' + start + '_' + end + '-' + parameter + '-' + string_number + '.pkl'
    else:
       if det_type=='geds': pkl_name = exp + '-' + period + '-' + run + '-' + datatype + '-' + parameter + '-string' + string_number + '.pkl'
       if det_type=='spms': pkl_name = exp + '-' + period + '-' + run + '-' + datatype + '-' + parameter + '-' + string_number + '.pkl'
...
def plot_par_vs_time_2d(raw_files, det_list, time_cut, det_type, string_number, det_dict, pdf=None):
    ...
    if 'OB' in string_number:  
       fig, ((ax1, ax2, ax3, ax4, ax5), (ax6, ax7, ax8, ax9, ax10), (ax11, ax12, ax13, ax14, ax15), (ax16, ax17, ax18, ax19, ax20)) = plt.subplots(4, 5, sharex=True, sharey=True)
       ax_list = [ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9, ax10, ax11, ax12, ax13, ax14, ax15, ax16, ax17, ax18, ax19, ax20]
    ...
    for detector in det_list:
        wf_array  = lh5.load_nda(raw_files, ['values'], detector+'/raw/waveform', verbose=False)['values']
        ...
        # select the channel
        utime_array = analysis.build_utime_array(raw_files, detector, 'spms') # shifted timestamps (puls events are not removed)
        utime_array, wf_array = analysis.time_analysis(utime_array, wf_array, time_cut)

        # calculate the gain
        if parameter == 'gain': par_array = parameters.spms_gain(wf_array) 
        if len(par_array) == 0 and len(utime_array) == 0: continue
        ...
        # 2D-plot
        H, xedges, yedges = np.histogram2d(utime_array, par_array, bins=[200,200], range=[[utime_array[0], utime_array[-1]], [0,300]])
        ...
        ax_list[ax_idx].pcolor(xedges_datetime, yedges, H.T, norm=mpl.colors.LogNorm(), cmap='magma')
        if 'OB' in string_number:
           ax_list[ax_idx].set_title(f'{detector} - FC: {card},{ch_orca} ({crate})', fontsize=7, y=0.93)
           if ax_idx==0 or ax_idx==5 or ax_idx==10 or ax_idx==15: ax_list[ax_idx].set(ylabel='Gain [ADC]')
           if ax_idx==15 or ax_idx==16 or ax_idx==17 or ax_idx==18 or ax_idx==19: ax_list[ax_idx].set(xlabel=f'{j_config[10]["frmt"]} (UTC)')
        if 'IB' in string_number:
           ax_list[ax_idx].set_title(f'{detector} - FC: {card},{ch_orca} ({crate})', fontsize=7, y=0.95)
           if ax_idx==0 or ax_idx==3 or ax_idx==6: ax_list[ax_idx].set(ylabel='Gain [ADC]')
           if ax_idx==6 or ax_idx==7 or ax_idx==8:ax_list[ax_idx].set(xlabel=f'{j_config[10]["frmt"]} (UTC)')
