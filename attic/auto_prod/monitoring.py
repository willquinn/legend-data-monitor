...
def get_calib_pars(period, run_list, channel, partition, escale=2039, fit='linear', path='/data2/public/prodenv/prod-blind/tmp/auto'):#'/data2/public/prodenv/prod-blind/ref/v02.00'):
    sto = lh5.LH5Store()
    ...
    for run in run_list:
        prod_ref = path
        timestamp = os.listdir(f'{path}/generated/par/{tier}/cal/{period}/{run}')[-1].split('-')[-2]
        if tier == 'pht':
            pars = json.load(
                open(
        ...
        if tier == 'pht':
            try:
                fep_peak_pos =  pars[channel]['results']['ecal']['cuspEmax_ctc_cal']['pk_fits']['2614.5']['parameters_in_ADC']['mu']
            ...
            try:
                fep_peak_pos =  pars[channel]['ecal']['cuspEmax_ctc_cal']['peak_fit_pars']['2614.5'][1]
                fep_gain = fep_peak_pos/2614.5
            except:
                fep_peak_pos=0
                fep_gain=0
        ...
        if tier == 'pht':
            try:
                if fit == "linear":
                    Qbb_fwhm = pars[channel]['results'][key_result]['cuspEmax_ctc_cal']['eres_linear']['Qbb_fwhm(keV)']
                    Qbb_fwhm_quad = pars[channel]['results'][key_result]['cuspEmax_ctc_cal']['eres_quadratic']['Qbb_fwhm(keV)']
                else:
                    Qbb_fwhm = pars[channel]['results'][key_result]['cuspEmax_ctc_cal']['eres_quadratic']['Qbb_fwhm(keV)']
            except:
                Qbb_fwhm = np.nan
        else:    
            try:
        ...
        if tier == 'pht':
            try:
                cal_const_a = pars[channel]['pars']['operations']['cuspEmax_ctc_cal']['parameters']['a']
                cal_const_b = pars[channel]['pars']['operations']['cuspEmax_ctc_cal']['parameters']['b']
                cal_const_c = pars[channel]['pars']['operations']['cuspEmax_ctc_cal']['parameters']['c']
                fep_cal = cal_const_c + fep_peak_pos * cal_const_b + cal_const_a * fep_peak_pos ** 2
            except:
                fep_cal = np.nan
        else:
            try:
                cal_const_a = pars[channel]['operations']['cuspEmax_ctc_cal']['parameters']['a']
                cal_const_b = pars[channel]['operations']['cuspEmax_ctc_cal']['parameters']['b']
                if period in ['p07'] or (period == 'p06' and run == 'r005'):
                    cal_const_c = pars[channel]['operations']['cuspEmax_ctc_cal']['parameters']['c']
                    fep_cal = cal_const_c + fep_peak_pos * cal_const_b + cal_const_a * fep_peak_pos ** 2
                else:
        ...
        if run not in os.listdir(f'{prod_ref}/generated/tier/dsp/phy/{period}'):
            ...
            run_files = sorted(os.listdir(f'{prod_ref}/generated/tier/dsp/cal/{period}/{run}/'))
            run_end_time = pd.to_datetime(
                sto.read_object("ch1027201/dsp/timestamp",
                                f'{prod_ref}/generated/tier/dsp/cal/{period}/{run}/' + run_files[-1])[0][-1],
                unit='s'
            )
            ...
            run_files = sorted(os.listdir(f'{prod_ref}/generated/tier/dsp/phy/{period}/{run}/'))
            run_start_time = pd.to_datetime(
                sto.read_object("ch1027201/dsp/timestamp",
                                f'{prod_ref}/generated/tier/dsp/phy/{period}/{run}/' + run_files[0])[0][0],
                unit='s'
            )
            run_end_time = pd.to_datetime(
                sto.read_object("ch1027201/dsp/timestamp",
                                f'{prod_ref}/generated/tier/dsp/phy/{period}/{run}/' + run_files[-1])[0][-1],
                unit='s'
            )
...
def stability(phy_mtg_data, output_folder, dataset, chmap, str_chns, xlim_idx, partition=False, quadratic=False, zoom=True):
    ...
    for index_i in tqdm(range(len(period_list))):
        ...
        geds_df_cuspEmax_abs, geds_df_cuspEmax_abs_corr, puls_df_cuspEmax_abs = get_dfs(phy_mtg_data, period, run_list)
        ...
        for index_j in tqdm(range(len(string_list))):
            ...
            for index_k in range(len(channel_list)):
                ...
                pulser_data = get_pulser_data(period, dfs, int(channel.split('ch')[-1]), escale=2039)
                if pulser_data is None:
                    continue
                
                fig, ax = plt.subplots(figsize=(12,4))
                
                pars_data = get_calib_pars(period, run_list, channel, partition, escale=2039)

                if channel != 'ch1120004':  

                    # plt.plot(pulser_data['ged']['cusp_av'], 'C0', label='GED')
                    plt.plot(pulser_data['pul_cusp']['kevdiff_av'], 'C2', label='PULS01')
                ...
                fig.suptitle(f'period: {period} string: {string} ged: {chmap.map("daq.rawid")[int(channel[2:])]["name"]}')
                ...
                plt.xlim(t0[0] - pd.Timedelta(hours=20), t0[-xlim_idx] + pd.Timedelta(days=7))
                ...
                plt.savefig(f'{output_folder}/{period}/st{string}/{period}_string{string}_pos{chmap.map("daq.rawid")[int(channel[2:])]["location"]["position"]}_{chmap.map("daq.rawid")[int(channel[2:])]["name"]}_gain_shift.pdf')
                plt.close(fig)